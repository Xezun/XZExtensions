...
@property (nonatomic, readonly) BOOL xz_containsDuplicateObjects;
...
- (NSMutableArray *)xz_map:(id _Nonnull (^NS_NOESCAPE)(ObjectType obj, NSInteger idx, BOOL *stop))transform;

/// 遍历并将结果返回结果生成新的数组，返回 nil 则该结果被忽略。
/// @note 生成的新数组的元素个数可能比原数少。
/// @param transform 遍历数组元素的块函数
/// @returns 块函数返回值组成的数组，可能比原数组元素少
- (NSMutableArray *)xz_compactMap:(id _Nullable (^NS_NOESCAPE)(ObjectType obj, NSInteger idx, BOOL * _Nonnull stop))transform;

/// 过滤掉 isIncluded 返回 NO 的对象。
/// @param isIncluded 遍历数组的块函数，返回 NO 则元素被过滤（不添加到新数组中）
- (NSMutableArray<ObjectType> *)xz_filter:(BOOL (^NS_NOESCAPE)(ObjectType obj, NSInteger idx))isIncluded;

/// 返回第一个符合条件的元素。
/// @param isIncluded 判断是否符合条件的块函数
- (ObjectType)xz_first:(BOOL (^NS_NOESCAPE)(ObjectType obj, NSInteger idx))isIncluded;

/// 返回最后一个符合条件的元素。
/// @param isIncluded 判断是否符合条件的块函数
- (ObjectType)xz_last:(BOOL (^NS_NOESCAPE)(ObjectType obj, NSInteger idx))isIncluded;

/// 是否包含 isIncluded 返回 YES 的元素。
/// @param isIncluded 遍历数组的块函数，返回 YES 表示匹配到元素并终止遍历
- (BOOL)xz_contains:(BOOL (^NS_NOESCAPE)(ObjectType obj, NSInteger idx))isIncluded;

/// 查找第一个符合条件的对象。如果没有找到，返回 NSNotFound 值。
/// @param predicate 判断条件
- (NSInteger)xz_firstIndex:(BOOL (^NS_NOESCAPE)(ObjectType obj))predicate;
...
- (void)xz_differenceFromArray:(NSArray *)oldArray inserts:(nullable NSMutableIndexSet *)inserts deletes:(nullable NSMutableIndexSet *)deletes changes:(nullable NSMutableDictionary<NSNumber *, NSNumber *> *)changes remains:(nullable NSMutableIndexSet *)remains;

@end

@interface NSMutableArray<ObjectType> (XZKit)

- (nullable ObjectType)xz_removeLastObject;
- (ObjectType)xz_removeObjectAtIndex:(NSUInteger)index;
